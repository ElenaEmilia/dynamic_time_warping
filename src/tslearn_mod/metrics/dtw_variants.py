...
@njit()
def njit_zc_accumulated_matrix(s1, s2, mask, mv_idx):
    """Compute the accumulated cost matrix score between two time series.
    ...
    cum_sum = numpy.full((l1 + 1, l2 + 1), numpy.inf)
    cum_sum[0, 0] = 0.
    mv_idx_inc = numpy.copy(mv_idx)
    mv_idx_inc += 1

    for i in range(l1):
        for j in range(l2):
            if numpy.isfinite(mask[i, j]):
                if j not in mv_idx_inc:
                    cum_sum[i + 1, j + 1] = _local_squared_dist(s1[i], s2[j])
...
def njit_dtw(s1, s2, mask):
    ...
    return numpy.sqrt(acc_cost_mat[-1, -1])


@njit(nogil=True)
def njit_zc_dtw(s1, s2, mask, mv_idx):
    """Compute the dynamic time warping score between two time series.
    ...
    """
    acc_cost_mat = njit_zc_accumulated_matrix(s1, s2, mask=mask, mv_idx=mv_idx)
    return numpy.sqrt(acc_cost_mat[-1, -1])
...
@njit()
def _return_zc_path(acc_cost_mat, mv_idx):
    sz1, sz2 = acc_cost_mat.shape
    path = [(sz1 - 1, sz2 - 1)]
    while path[-1] != (0, 0):
        i, j = path[-1]
...
@njit()
def _return_zc_imputation(acc_cost_mat, mv_idx, impute=None):
    if impute is not None:
        source = numpy.copy(impute[0])
        target = numpy.copy(impute[1])
    sz1, sz2 = acc_cost_mat.shape
    path = [(sz1 - 1, sz2 - 1)]
    while path[-1] != (0, 0):
        i, j = path[-1]
        ...
        if i == 0:
            ...
            if argmin == 0:
                ...
                path.append((i - 1, j))
    if (impute is not None) and (0 in mv_idx):
        target[0] = source[0]
    return numpy.ravel(target)
...
def zc_dtw_path(s1, s2, global_constraint=None, sakoe_chiba_radius=None,
             itakura_max_slope=None):
    r"""Compute Dynamic Time Warping (DTW) similarity measure between
    (possibly multidimensional) time series and return both the path and the
    similarity.
    ...
    """
    s1 = to_time_series(s1, remove_nans=False)
    s2 = to_time_series(s2, remove_nans=False)
    ...
    if numpy.isnan(s2).any():
        missing_value_indices = numpy.argwhere(numpy.isnan(s2))[:,0]
    else:
        raise ValueError("No missing values in `s2` detected, please use default DTW")

    mask = compute_mask(
        s1, s2, GLOBAL_CONSTRAINT_CODE[global_constraint],
        sakoe_chiba_radius, itakura_max_slope
    )
    acc_cost_mat = njit_zc_accumulated_matrix(s1, s2, mask=mask, mv_idx=missing_value_indices)
    path = _return_zc_path(acc_cost_mat, mv_idx=missing_value_indices)
    return path, numpy.sqrt(acc_cost_mat[-1, -1])


def zc_dtw_imputation(source, target, global_constraint=None, sakoe_chiba_radius=None,
             itakura_max_slope=None):
    r"""Compute Dynamic Time Warping (DTW) similarity measure between
    (possibly multidimensional) time series and return both the path and the
    similarity.
    ...
    list of integer pairs
        Matching path represented as a list of index pairs. In each pair, the
        first index corresponds to source and the second one corresponds to target
    ...
    """
    source = to_time_series(source, remove_nans=False)
    target = to_time_series(target, remove_nans=False)
    ...
    if numpy.isnan(target).any():
        missing_value_indices = numpy.argwhere(numpy.isnan(target))[:,0]
    else:
        raise ValueError("No missing values in `target` detected, please use default DTW")

    mask = compute_mask(
        source, target, GLOBAL_CONSTRAINT_CODE[global_constraint],
        sakoe_chiba_radius, itakura_max_slope
    )
    acc_cost_mat = njit_zc_accumulated_matrix(source, target, mask=mask, mv_idx=missing_value_indices)
    imputed = _return_zc_imputation(acc_cost_mat, mv_idx=missing_value_indices,
                                    impute=(source, target))
    return imputed, numpy.sqrt(acc_cost_mat[-1, -1])
...
def zc_dtw(s1, s2, global_constraint=None, sakoe_chiba_radius=None,
        itakura_max_slope=None):
    r"""Compute Dynamic Time Warping (DTW) similarity measure between
    (possibly multidimensional) time series and return it.
    ...
    mask = compute_mask(
        ...
        sakoe_chiba_radius=sakoe_chiba_radius,
        itakura_max_slope=itakura_max_slope)
    return njit_zc_dtw(s1, s2, mask=mask, mv_idx=missing_value_indices)
